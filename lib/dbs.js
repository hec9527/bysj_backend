...
// 360图片表
items.image = {
    check: `SELECT table_name FROM information_schema.TABLES WHERE table_name = '${config.table_image}' and table_schema='${config.db_name}';`,
    init: `CREATE TABLE IF NOT EXISTS ${config.table_image}(
    id INT(20) NOT NULL AUTO_INCREMENT PRIMARY KEY, url varchar(256) NOT NULL,
    resolution varchar(15), category varchar(50), tag varchar(256), gary64 char(64), FULLTEXT(category));`,
    msg: '图片表',
};

// 必应美图
items.biying = {
    check: `SELECT table_name FROM information_schema.TABLES WHERE table_name ='${config.table_biying}' and table_schema='${config.db_name}';`,
...
// 金山词典，每日英语
items.english = {
    check: `SELECT table_name FROM information_schema.TABLES WHERE table_name ='${config.table_english}' and table_schema='${config.db_name}';`,
...
// 用户表
items.user = {
    check: `SELECT table_name FROM information_schema.TABLES WHERE table_name ='${config.table_user}' and table_schema='${config.db_name}';`,
...
// 爬虫表  增加爬虫模块需要增加数据
items.spider = {
    check: `SELECT table_name FROM information_schema.TABLES WHERE table_name ='${config.table_spider}' and table_schema='${config.db_name}';`,
