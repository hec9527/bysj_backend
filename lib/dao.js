...
const cnnPool = mysql.createPool({
    ...
    database: config.dbDatabase || 'bysj1', // 优先使用数据库配置中的数据库名 而不是全局的数据库名
...
module.exports = {
    ...
    updateUserInfo(param, callback) {
        let sql = `UPDATE ${tables.table_user} SET `;
        Object.keys(param).forEach((key) => {
            if (![null, undefined, NaN].includes(param[key]) && key !== 'id') {
                sql += ` ${String(key).replace("'", '')}='${String(param[key]).replace("'", '')}', `;
            }
        });
        sql = sql.substr(0, sql.length - 2);
        sql += ` where id='${String(param.id).replace("'", '')}';`;
        myQuery(sql, callback);
    },
    ...
    addUserInfo(param, callback) {
        const sql = `INSERT INTO ${tables.table_user} (count,passwd,name,sex) values('${param.count}', '${param.passwd}', '${param.name}', '${param.sex}');`;
        myQuery(sql, callback);
    },
     ...
     * @param {Function} callback
     */
    getImageInfo(count, len, callback) {
        const sql = `SELECT * FROM ${tables.table_image} LIMIT ${count},${len};`;
        myQuery(sql, callback);
    },
     ...
     * @param {Function} callback
     */
    getImageInfoCount(callback) {
        const sql = `SELECT COUNT(*) AS TOTAL FROM ${tables.table_image};`;
        myQuery(sql, callback);
    },
     ...
     * @param {Function} callback
     */
    delImageInfo(id, callback) {
        const sql = `DELETE FROM ${tables.table_image} WHERE id='${id}';`;
        myQuery(sql, callback);
    },
};
