...
module.exports = {
    ...
    getNewImage: function(begin, len, callback) {
        const sql = `SELECT * FROM images ORDER BY fUpTime desc LIMIT ${begin},${begin +
            len};`;
        myQuery(sql, callback);
    },
    ...
    getHotImage: function(begin, len, callback) {
        const sql = `SELECT * FROM images ORDER BY fStar desc LIMIT ${begin},${begin +
            len};`;
        myQuery(sql, callback);
    },
    ...
    getImageByUser: function(user, begin, len, callback) {
        const sql = `SELECT * FROM images WHERE userAccount=${user} LIMIT ${begin},${begin +
            len};`;
        mysql(sql, callback);
    },
    ...
    getImageBySize: function(size, begin, len, callback) {
        var size = ['720p', '1080p', '2k', '4k', '8k'].includes(size)
            ? size
            : 'other';
        const sql = `SELECT * FROM imagepath WHERE fResolution=${size} LIMIT ${begin},${begin +
            len};`;
        myQuery(sql, callback);
    },
