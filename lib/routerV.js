...
router.use(async (ctx, next) => {
    // 过滤用户token验证
    if (config.enableVerify && !jwt.verifyToken(ctx.header.authorization)) {
...
// 获取用户个人信息
router.get('/get/userInfo', async (ctx) => {
    // id, count, name, iat, exp
    const info = jwt.decodeToken(ctx.header.authorization);
    await new Promise((res, rej) => {
        dao.getUserInfoAll(info.id, (data) => (data ? res(data) : rej(data)));
    }).then(
        (res) => {
            if (res && res.length === 1) {
                const data = res[0];
                // 删除用户密码字段
                Reflect.deleteProperty(data, 'passwd');
                ctx.statusCode = 200;
                ctx.body = {
                    code: 0,
                    msg: 'ok',
                ...
                ctx.statusCode = 500;
                logger.error('获取用户所有信息出错：', res);
            }
        },
        (rej) => {
            ctx.statusCode = 500;
            logger.error('获取用户所有信息错误：', rej);
        }
    );
});

// 通过文件路径获取图片    任意图
router.get('/get/assets/image/data', async (ctx) => {
...
// 获取单个图片的数据   缩略图
router.get('/get/assets/image/nail/:fileName', async (ctx) => {
...
// 获取单个图片的数据    高清图
router.get('/get/assets/image/hd/:fileName/:fSize?', async (ctx) => {
...
router.post('/post/user/info', async (ctx) => {
    await new Promise((resolve) => {
        dao.getUserInfo(jwt.decodeToken(ctx.header.token)['userAccount'], (data) => {
            data = data[0];
...
router.post('/post/user/update', async (ctx) => {
    await new Promise((resolve) => {
        ...
        dao.setUserInfo(userAccount, tool.objToString(ctx.request.body), (data) => {
...
router.get('/get/list/new/:begin/:len?', async (ctx) => await getJSON(ctx, dao.getNewImage));

// 分页查询   获取热度最高的图片，给定起始位置和长度   len允许值1-300
router.get('/get/list/hot/:begin/:len?', async (ctx) => await getJSON(ctx, dao.getHotImage));

// 分页查询   根据分辨率获取图片，给定分辨率大小，起始位置，长度
router.get('/get/list/size/:size/:begin/:len?', async (ctx) => await getJSONByParmas(ctx, dao.getImageBySize));

// 分页查询   根据图片的标签获取图片，给定图片的类型，起始位置，长度
router.get(
    '/get/list/tag/assets/image/:param/:begin/:len?',
    async (ctx) => await getJSONByParmas(ctx, dao.getImageByTag)
);

// 分页查询   根据标签查询用户，给定查询的方式，起始位置，长度
router.get('/get/list/tag/user/:param/:begin/:len?', async (ctx) => await getJSONByParmas(ctx, dao.getUserByTag));

// 分页查询   获取用户上传的所有图片 并且按照时间排序
router.get('/get/list/user/:user/:begin/:len?', async (ctx) => await getJSONByParmas(ctx, dao.getImageByUser));
...
async function getJSONByParmas(ctx, callback) {
    try {
        ...
        if (len < 0 || len > 300 || begin < 0) throw 'Unexpected parmas, 0 <= len <= 300，and begin >= 0';
        await new Promise((resolve, reject) => {
